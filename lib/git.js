...
export async function pushNotes(repositoryUrl, execaOptions) {
  writeFileSync(`.store/notes.json`, JSON.stringify(notesStore));
  await execa("git", ["add", ".store"], execaOptions);
  await execa("git", ["commit", "-m", "Update notes"], execaOptions);
  await execa("git", ["push", repositoryUrl, `HEAD:${branch}`], execaOptions);
...
export async function getNote(ref, execaOptions) {
  try {
    return JSON.parse(readFileSync(`.store/notes.json`)) || {};
